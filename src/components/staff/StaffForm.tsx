...
export type StaffFormValues = {
  ...
  is_active: boolean
  // Omit to leave unchanged on edit; pass [] to clear; or array to set
...
const StaffForm: React.FC<StaffFormProps> = ({ initial, onSubmit, onCancel, submitting }) => {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    // Only include schedule if explicitly enabled
    if (!scheduleEnabled) {
      delete (payload as any).schedule
    } else {
      // If enabled but no items, send [] to clear (or create with none)
    ...
    <form onSubmit={submit} className="space-y-4">
      ...
      <div className="border-t pt-4">
        <div className="flex items-center gap-2 mb-2">
          ...
          />
          <label htmlFor="schedule_enabled" className="text-sm font-medium text-gray-700">Include schedule (day_of_week 0–6)</label>
        </div>
        {scheduleEnabled && (
          <ScheduleInput

  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        {data.brand && <div className="text-sm text-gray-600">Brand: {data.brand}</div>}
        {data.sku && <div className="text-sm text-gray-600">SKU: {data.sku}</div>}
      </div>
      <div className="grid grid-cols-2 gap-4 text-sm">
        <div>
          <div className="text-gray-500">Retail Price</div>
          <div className="font-medium">${Number(data.retail_price).toFixed(2)}</div>
        </div>
        <div>
          <div className="text-gray-500">Cost Price</div>
          <div className="font-medium">{data.cost_price != null ? `$${Number(data.cost_price).toFixed(2)}` : '—'}</div>
        </div>
        <div>
          <div className="text-gray-500">Stock</div>
          <div className="font-medium">{data.stock_quantity}</div>
        </div>
        <div>
          <div className="text-gray-500">Low stock threshold</div>
          <div className="font-medium">{data.low_stock_threshold}</div>
        </div>
      </div>
      {data.description && (
        <div>
